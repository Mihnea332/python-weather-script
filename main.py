...
def trigger_notification():
    ...
    if not weather_message:
        raise HTTPException(status_code=500, detail="We couldn't extract the weather information.")
        
    success = send_ntfy(weather_message)
    
    if not success:
        raise HTTPException(status_code=500, detail="Couldn't send ntfy message")
        
    return {"success": True, "message": "The notification was sent successfully via ntfy!"}
